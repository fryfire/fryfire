...
/**
 * Returns the pixel from the given image.
 *
 * @param image - The image (Must already be fully loaded).
 * @return The image data.
 */
export function getImageData(image: HTMLImageElement): ImageData {
    const ctx = getRenderingContext(createCanvas(image.naturalWidth, image.naturalHeight), "2d");
    ctx.drawImage(image, 0, 0, image.width, image.height);
    return ctx.getImageData(0, 0, image.width, image.height);
}

/**
 * Translates the given canvas to a full pixel. This is useful for drawing filled rectangles at exact pixel boundaries.
 *
 * @param ctx - The 2D canvas rendering context.
 */
export function translateToFullPixel(ctx: CanvasRenderingContext2D): void {
...
/**
 * Translates the given canvas to a half pixel. This is useful for drawing lines at exact pixel boundaries.
 *
 * @param ctx - The 2D canvas rendering context.
 */
export function translateToHalfPixel(ctx: CanvasRenderingContext2D): void {
    const transform = ctx.getTransform();
    ctx.translate(
        Math.round(transform.e) - transform.e + 0.5,
        Math.round(transform.f) - transform.f + 0.5
    );
}
