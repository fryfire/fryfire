...
export interface SceneNodeArgs {
    ...
    /**
     * Set to true to use the app time instead of game time so this animation is not affected by game speed or pause.
     */
    useAppTime?: boolean;
}
...
export class SceneNode<T extends Game = Game> {
    ...
    /** True to use app time, false to use game time which is affected by game speed and pause. */
    protected readonly useAppTime: boolean;
    ...
    public constructor({ id = null, x = 0, y = 0, width = 0, height = 0, anchor = Direction.CENTER,
            ...
            collisionMask = 0, useAppTime = false, cameraTargetOffset }: SceneNodeArgs = {}) {
        this.id = id;
     ...
     */
    private updateAnimations(timer: Timer): void {
        const animations = this.animations;
        let numAnimations = animations.length;
        let i = 0;
        while (i < numAnimations) {
            if (animations[i].update(this, timer)) {
     ...
     */
    protected updateAll(timer: Timer): number {
        // Update this node and run animations
        const postUpdate = this.update(timer);
        this.updateAnimations(timer);

        // Update child nodes
        const layers = this.updateChildren(timer) | this.getEffectiveLayer();
     ...
     * @return Bit mask with used layers.
     */
    protected updateChildren(timer: Timer): number {
        let layers = 0;
        this.forEachChild(child => {
            layers |= child.updateAll(timer);
        });
        return layers;
    }
     ...
     */
    protected update(timer: Timer): void | (() => void) {}
