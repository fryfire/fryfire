...
export class Scenes<T extends Game> {
    ...
    private sceneCache = new WeakMap<SceneConstructor<T>>();
    private scenes: Scene<T, unknown>[] = [];
    private sortedScenes: Scene<T, unknown>[] = [];

    public constructor(public readonly game: T) {}

    private createScene<A>(sceneClass: SceneConstructor<T>): Scene<T, A> {
    ...
    public async pushScene<A>(sceneClass: SceneConstructor<T>, args: void): Promise<void>;
    public async pushScene<A>(sceneClass: SceneConstructor<T>, args: A): Promise<void>;
    public async pushScene<A>(sceneClass: SceneConstructor<T>, args: A): Promise<void> {
    ...
    public update(timer: Timer): void {
        this.sortedScenes.forEach(scene => {
            scene.currentTransition?.update(timer);
            scene.update(timer);
        });
    }
